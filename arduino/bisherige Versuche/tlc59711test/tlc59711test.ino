...
#define NUM_TLC59711 1 // 2 vorher 
...
void colorWipe(uint16_t r, uint16_t g, uint16_t b, uint8_t wait) { // IN DIESER FUNKTION Vibriert es.
  for(uint16_t i=0; i<8*NUM_TLC59711; i++) {                       // hier ist die LED von 0 bis 7
      tlc.setLED(i, r, g, b);
      tlc.write();
      delay(wait);
  }
